...
1. `company/config/engineering-org.yaml`을 읽는다 (이번 실행에선 안 쓰지만 다음 단계가 참조할 값이니 존재 확인).
...
3. 진단:
   ...
   ```
4. 발견한 각 항목을 `company/issues/ISSUE-<YYYYMMDD>-<NN>.yaml`로 만든다. `company/issues/TEMPLATE.yaml`을 복사해서 채운다. `state: DETECTED`.
...
7. `company/reports/<오늘>.md`에 사람이 읽을 요약을 쓴다: 오늘 새로 등록된 이슈 목록, 각 severity, 어제 대비 변화.
8. 마지막에 `ls -la company/issues/`로 실제 생성 확인.
